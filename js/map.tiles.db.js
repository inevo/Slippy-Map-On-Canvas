        ...
        window.slippymap.extension.tiles.tilesCache = function (map) {
            var tilesCache = {
                    ...
                    if (window.openDatabase) {
                        ...
                        if (tilesCache.sqlite) {
                            tilesCache.sqlite.transaction(function (tx) {
                                ...
                                tx.executeSql("SELECT COUNT(*) FROM tiles", [], function (tx, result) {}, function (tx, error) {
                ...
                update: function (img) {
                    ...
                    tilesCache.sqlite.transaction(function (tx) {
                        tx.executeSql("INSERT INTO tiles (provider, x, y, z, timestamp, data) VALUES (?, ?, ?, ?, ?, ?)", [img.meta.provider, img.meta.x, img.meta.y, img.meta.z, now(), img.data], function (result) {
                        });
                    });
                },
                ...
                tileprovider: function (x, y, z, img) {
                    tilesCache.sqlite.transaction(function (tx) {
                        tx.executeSql("SELECT data FROM tiles WHERE provider = ? AND x = ? AND y = ? AND z = ?", [tilesCache.providerID, x, y, z], function (tx, result) {
                            if (!result.rows.length) {
                                ...
                                img.meta = {
                                    ...
                                    "y": y,
                                    "z": z,
                                    "provider": tilesCache.providerID
                                };
                                img.onload = function () {
                                    if(img.complete){
                                ...
                                img.src = result.rows.item(0).data;
                                map.renderer.refresh();
                            }
                        });
                    });
